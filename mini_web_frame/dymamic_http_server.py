...
class WSGIServer(object):
    def __init__(self):
        self.tcp_server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # 创建
        self.tcp_server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # 设定套接字选项
        self.tcp_server_socket.bind(("", 8888))  # 绑定
        self.tcp_server_socket.listen(128)  # 监听
    ...
    def run_forever(self):
        while True:
            new_socket, client_addr = self.tcp_server_socket.accept()  # 等待客户端
